...
class LinearLayer(object):
    ...
    def forward(self, X):
        ...
        self.output = X @ self.W + np.tile(b,(X.shape[0],1))
        return self.output
    ...
    def grad(self, X, delta_next):
        ...
        """
        #print("s {}, i {}, u {}".format(X.shape[0],self.W.shape[0],self.W.shape[1]))
        #print(delta_next.shape)

        return [X.T @ delta_next ,(np.ones((1,X.shape[0])) @ delta_next).reshape(-1)]
...
if __name__ == "__main__":
    ...
    lin = LinearLayer(4, 4, rng, "lin_layer")
    print(lin.forward(X))
